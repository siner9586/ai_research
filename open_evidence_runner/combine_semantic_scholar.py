...
def main():
    ...
    summaries=[json.loads(x.read_text(encoding='utf-8')) for x in sorted(a.input.glob('**/completion/semantic_scholar_*_summary.json'))]
    records={}
    for path in sorted(a.input.glob('**/normalized/semantic_scholar/*/paper_master_semantic_scholar.jsonl.gz')):
        with gzip.open(path,'rt',encoding='utf-8') as f:
            for line in f:
                if not line.strip(): continue
                row=json.loads(line); key=row.get('doi_normalized') or row.get('s2_paper_id') or row['source_external_id']
                if key in records:
                    records[key]['query_groups']=sorted(set(records[key].get('query_groups',[]))|set(row.get('query_groups',[])))
                    records[key]['query_texts']=sorted(set(records[key].get('query_texts',[]))|set(row.get('query_texts',[])))
                    for field in ('abstract','oa_pdf_url','venue','authors','external_ids'):
                        if not records[key].get(field) and row.get(field): records[key][field]=row[field]
                else: records[key]=row
    master=a.output/'semantic_scholar_all_paper_master.jsonl.gz'
    with gzip.open(master,'wt',encoding='utf-8') as f:
        for key in sorted(records): f.write(json.dumps(records[key],ensure_ascii=False)+'\n')
    manifest=a.output/'semantic_scholar_all_search_queries.csv'; writer=None
    with manifest.open('w',encoding='utf-8-sig',newline='') as out:
        for path in sorted(a.input.glob('**/manifests/semantic_scholar_*_search_queries.csv')):
