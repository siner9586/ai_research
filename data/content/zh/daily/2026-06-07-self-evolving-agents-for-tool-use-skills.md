...
generated_at: "2026-06-07T00:16:24.846469+00:00"
page_type: "brief"
candidate_count: 18
featured_count: 6
mentions_count: 12
...
核心：这篇论文主要解决视觉语言模型在复杂视觉提示、风险场景和对齐失效情形下如何被系统评估的问题；方法上通过多模态安全评测套件组织图像、文本和风险提示的交叉测试，实现对模型图表、文档与视觉证据理解能力的压力检验；主要论点是摘要显示该工作把安全风险与多模态理解能力放在同一评测框架中观察，结论是否可迁移到真实产品场景仍需阅读全文确认。关键词：multimodal、safety evaluation、vision language models、alignment。代码/数据可用性需查看原文确认。
...
核心：这篇论文主要解决 RAG 系统在噪声证据、缺失引用和对抗性文档影响下如何保持问答可靠性的问题；方法上通过围绕噪声检索构造基准和失效案例来观察证据质量变化，实现对检索增强生成流程的鲁棒性评估；主要论点是摘要显示该工作把可靠性问题拆解为证据、引用和文档干扰等可检验环节，适合补充企业知识库问答的回归测试。关键词：RAG、retrieval、noisy evidence、citations。代码/数据可用性需查看原文确认。
...
## 其他值得关注
- [Preference Optimization for Safer Tool Agents](https://arxiv.org/abs/2606.00012)：涉及工具调用、执行反馈和可复用能力，可作为 Agent 工作流可靠性的补充线索。
- [Red Teaming Open Source LLM Guardrails](https://arxiv.org/abs/2606.00017)：涉及模型安全、护栏路由、风险分类或治理评测，可作为安全评测与治理工具链的补充线索。
- [Database Native Retrieval for Enterprise RAG](https://arxiv.org/abs/2606.00013)：涉及检索、知识库问答与证据可靠性，可作为 RAG 评测和企业知识系统的补充线索。
- [Chart Understanding for Vision Language Models](https://arxiv.org/abs/2606.00014)：涉及多模态模型中的任务、数据或系统线索，可作为后续跟进清单的一部分。
- [Agentic 3D Modeling through Code Execution](https://arxiv.org/abs/2606.00015)：涉及工具调用、执行反馈和可复用能力，可作为 Agent 工作流可靠性的补充线索。
- [Low Rank Adapters as Model Memory Probes](https://arxiv.org/abs/2606.00018)：涉及训练与后训练中的表征观察线索，可作为模型记忆和适配器分析的补充材料。
- [Mechanistic Attribution for Factual Editing](https://arxiv.org/abs/2606.00008)：涉及内部表征和事实编辑归因，可补充可解释性与模型编辑方向。
- [Video Diffusion Models Need Temporal Tests](https://arxiv.org/abs/2606.00010)：涉及视频生成的时间一致性和运动真实感，可补充生成模型评测方向。
- [Training Data Deduplication for Foundation Models](https://arxiv.org/abs/2606.00016)：涉及训练数据去重和基础模型数据治理，可补充数据工程方向。
- [Robotics Policies with Memory Grounded Planning](https://arxiv.org/abs/2606.00006)：涉及机器人策略、记忆和规划，可补充具身智能任务线索。
- [Open Speech Agent Benchmark](https://arxiv.org/abs/2606.00009)：涉及语音 Agent 和基准任务，可补充语音/音频评测方向。
- [Serving Quantized Models with Adaptive Batching](https://arxiv.org/abs/2606.00011)：涉及推理成本、延迟、吞吐和部署约束，可补充系统优化方向。
